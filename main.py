...
def test_rgb(file_name):
    ...
    print(type(retrieved_img.astype(np.uint8)))
    print(retrieved_img.astype(np.uint8).shape)
    im = Image.fromarray(retrieved_img.astype(np.uint8))
    im.save('retrieved_'+file_name)
    im2 = Image.fromarray(np.array(img).astype(np.uint8).astype(np.uint8))
    im2.save('compressed_'+file_name)
